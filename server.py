...
DB  = r"C:/Users/<username>/career/job-tracker.db"
OUT = r"C:/Users/<username>/career/dashboard.html"
...
def load_rows():
    ...
    cur = conn.cursor()
    cur.execute("SELECT * FROM reviewed_postings ORDER BY date ASC, id ASC")
...
def load_monitor_data():
    ...
    MONITOR_DB = r"C:/Users/<username>/career/monitor.db"
    empty = {
        "generated_at": "",
...
def compute(rows):
    ...
    surfaced = [r for r in rows if r["status"] not in DEAD]
    tier1    = [r for r in rows if (r["score_pct"] or 0) >= 65]
    applied  = [r for r in rows if r["status"] == "Applied"]
    active   = [r for r in rows if r["status"] in
                ("New", "Pending", "Borderline", "Reviewing")]

    # ── Funnel
    funnel = {
        "stages": ["Total Reviewed", "Surfaced", "Tier 1 (65%+)", "Applied"],
        "values": [total, len(surfaced), len(tier1), len(applied)],
    }
    ...
    # ── Application quality ratio
    status_counts = Counter(r["status"] for r in rows)
    status_order  = ["Applied", "Borderline", "Pending", "New", "Closed", "Pass"]
    ...
    pass_cats = [categorize_pass(r.get("verdict") or r.get("notes"))
                 for r in passed]
    ...
    first_week = week_start(rows[0]["date"]) if rows else now_str
    ...
    # ── Active pipeline
    pipeline_rows = sorted(surfaced, key=lambda r: (r["score_pct"] or 0), reverse=True)
    pipeline = [{
        "company": r["company"],
        "role":    r["role"],
        "score":   r["score_pct"],
    ...
    without_comp = total - len(with_comp)
    ...
    # ── Summary KPIs
    scored    = [r["score_pct"] for r in rows if r["score_pct"] is not None]
    ...
    summary = {
        ...
        "passed":     len(passed),
        "avg_score":  avg_score,
        "apply_rate": round(len(applied) / total * 100, 1) if total else 0,
        "generated":  datetime.now().strftime("%B %d, %Y  -  %I:%M %p"),
    }

    return summary, funnel, score_dist, quality, pass_freq, velocity, pipeline, comp, sources
...
def status_badge(s):
    ...
    colors = {
        ...
        "Borderline": ("#78350f", "#fcd34d"),
        "Pending":    ("#1e3a5f", "#93c5fd"),
        "New":        ("#1f2937", "#9ca3af"),
        "Pass":       ("#7f1d1d", "#fca5a5"),
        "Reviewing":  ("#2e1065", "#c4b5fd"),
...
def pipeline_rows_html(pipeline):
    ...
    for i, r in enumerate(pipeline):
        ...
        out.append(
            ...
            f'<td style="padding:8px 14px;font-size:12px;color:#64748b;">{r["date"]}</td>'
            f'</tr>'
        )
    return "\n".join(out)
...
def generate_html(summary, funnel, score_dist, quality, pass_freq,
                  velocity, pipeline, comp, sources, monitor_data=None):
    j = lambda d: json.dumps(d)

    table_rows = pipeline_rows_html(pipeline)
...
<style>
  *, *::before, *::after {{ box-sizing: border-box; margin: 0; padding: 0; }}
  ...
  .kpi {{
    ...
    padding: 20px 22px; border-left: 3px solid;
  }}
  .kpi .lbl {{ font-size: 10px; font-weight: 700; text-transform: uppercase;
               letter-spacing: .8px; color: #475569; margin-bottom: 10px; }}
...
<div class="main">
  ...
  <div class="kpi-row">
    <div class="kpi" style="border-color:#6366f1;">
      ...
      <div class="val" style="color:#818cf8;">{summary["total"]}</div>
      <div class="sub">Roles evaluated</div>
    </div>
    <div class="kpi" style="border-color:#34d399;">
      <div class="lbl">Applied</div>
      <div class="val" style="color:#34d399;">{summary["applied"]}</div>
      <div class="sub">Apply rate {summary["apply_rate"]}%</div>
    </div>
    <div class="kpi" style="border-color:#fbbf24;">
      <div class="lbl">Active Pipeline</div>
      <div class="val" style="color:#fbbf24;">{summary["active"]}</div>
      <div class="sub">Borderline / Pending / New</div>
    </div>
    <div class="kpi" style="border-color:#a78bfa;">
      <div class="lbl">Avg Match Score</div>
      <div class="val" style="color:#a78bfa;">{summary["avg_score"]}%</div>
      <div class="sub">{summary["passed"]} auto-passed</div>
    </div>
  </div>
  ...
  <div class="row-2-1">
    ...
    <div class="card">
      <h2>Application Quality Ratio</h2>
      <div class="chart-wrap" style="height:230px;">
  ...
  <div class="card row-full">
    <h2>Active Pipeline &mdash; {summary["active"] + summary["applied"]} roles</h2>
    <table>
      <thead>
        <tr>
...
<div id="view-agents" style="display:none;">
  <div class="main">
    ...
    <div class="row-1-1">
      ...
      <div class="card">
        <h2>Scan Performance Over Time</h2>
        <div class="chart-wrap" style="height:250px;">
  ...
  new Chart(AC('agScanChart'), {{
    ...
    data: {{
      ...
      datasets:[
        ...
        {{ label:'Tier 1', data: d.scan_history.map(r => r.tier1_count || 0), borderColor:'#34d399', tension:.3, pointRadius:4 }},
        {{ label:'Tier 2', data: d.scan_history.map(r => r.tier2_count || 0), borderColor:'#fbbf24', tension:.3, pointRadius:4 }},
        {{ label:'Tier 3', data: d.scan_history.map(r => r.tier3_count || 0), borderColor:'#f87171', tension:.3, pointRadius:4 }}
      ]
    }},
    options:{{
      responsive:true, maintainAspectRatio:false,
...
def run_tests():
    ...
    # ── Schema ─────────────────────────────────────────────────────────────────
    expected_cols = {"id", "date", "company", "role", "score_pct", "status",
                     "comp", "link", "notes"}
    try:
    ...
    known_statuses = {"Applied", "Borderline", "New", "Pass", "Pending", "Reviewing", "Closed"}
    ...
    bad_dates = [r for r in rows if week_start(r.get("date")) is None]
    if bad_dates:
    ...
    try:
        t0 = datetime.now()
        summary, funnel, score_dist, quality, pass_freq, velocity, pipeline, comp, sources = compute(rows)
    ...
    # ── Active Pipeline ────────────────────────────────────────────────────────
    DEAD = ("Pass", "Closed")
    surfaced = [r for r in rows if r.get("status") not in DEAD]
    if surfaced:
        ok("Active Pipeline", f"{len(surfaced)} surfaced row(s)")
    else:
        fail("Active Pipeline", "surfaced rows list is empty")
    ...
    class Handler(BaseHTTPRequestHandler):
        def do_GET(self):
            try:
                ...
                print(f"[ERROR] Exception in do_GET:\n{tb}")
                error_body = (
                    f"<pre style='background:#1e293b;color:#fca5a5;"
                    f"padding:24px;font-size:13px;'>"
                ...
                try:
                    ...
                    pass  # headers may already be sent; swallow secondary error
