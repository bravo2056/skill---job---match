...
## Gmail Rules
Bulk email operations (scanning the `Job search 2026` label, relabeling
processed threads to `Job search 2026/Scanned`) run through `email-scanner.py`
and `run-scan.py` using the Gmail API direct path (OAuth token at
`token.json`, scope `gmail.modify`).

Ad-hoc Gmail queries from the agent (e.g., "check my latest from X") use
the Gmail MCP:
- `search_threads`
- `get_thread`
- `list_labels`
...
- Gmail label modification is allowed ONLY via the `email-scanner.py` script's
  Gmail API path (using `token.json`, scope `gmail.modify`). The agent must
  not call label modification through any other method.
- Never call `get_thread` (Gmail MCP) for bulk email scanning. Bulk reads go
  through `email-scanner.py` + `run-scan.py` only. MCP `get_thread` is allowed
  only for ad-hoc queries about a single specific email.
